...
interface DiscoveryZapDialogProps {
  target: Event;
      ...
      <>
        <div className="grid gap-3 px-4 py-4 w-full overflow-hidden">
          ...
          <div className="space-y-1">
            <Label htmlFor="zapvertisement-message">Zapvertisement Message</Label>
            <Textarea
              id="zapvertisement-message"
              placeholder="Add your message here (e.g., 'Check out my project!', 'Thanks for your content!', etc.)"
              value={comment}
            ...
            <p className="text-xs text-muted-foreground">
              This message will be attached to your zap and visible to the recipient.
            </p>
          </div>
        </div>
...
export function DiscoveryZapDialog({ target, children, className, onZapSuccess }: DiscoveryZapDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      // Set default zapvertisement message
      setComment('Discovered you on Nostr! Keep creating great content! ⚡');
    }
  }, [open]);
        ...
        >
          <DrawerHeader className="text-center relative">
            ...
            <DrawerTitle className="text-lg break-words pt-2">
              {invoice ? 'Lightning Payment' : 'Send a Zapvertisement'}
            </DrawerTitle>
            <DrawerDescription className="text-sm break-words text-center">
              {invoice ? (
                'Pay with Bitcoin Lightning Network'
              ) : (
                'Send sats with a custom message to promote yourself or say thanks!'
              )}
            </DrawerDescription>
          </DrawerHeader>
